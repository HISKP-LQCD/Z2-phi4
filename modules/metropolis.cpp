...
double metropolis_update(Viewphi& phi, cluster::IO_params params, RandPoolType& rand_pool, ViewLatt sectors) {
    ...
    const size_t nb_of_hits = params.data.metropolis_local_hits;
    const double g = params.data.g;
    // const bool split_g = params.data.split_g;
    // auto &phi=*field;
    const double kappa[2] = { params.data.kappa0, params.data.kappa1 };
    const int L[4] = { params.data.L[0], params.data.L[1], params.data.L[2], params.data.L[3] };
    ...
    for (int color = 0; color < 3; color++) {
        ...
        Kokkos::parallel_for("lattice metropolis loop", sectors.size[color], KOKKOS_LAMBDA(size_t xx) {
            ...
            compute_neibourgh_sum(neighbourSum, phi, x, L);
            ...
            if (test == 1) {
                ...
                for (size_t dir = 0; dir < dim_spacetime; dir++) { // dir = direction
                    neighbourSum1[0] += polar1( phi(0, hop(x, dir + dim_spacetime))) + polar1( phi(0, hop(x, dir)));
                    neighbourSum1[1] += polar1( phi(1, hop(x, dir + dim_spacetime))) + polar1( phi(1, hop(x, dir)));
                }
            ...
            for (size_t hit = 0; hit < nb_of_hits; hit++) {
                ...
                double dS = -2. * kappa[0] * ( polar1(- phi(0, x)) * neighbourSum[0] * (polar1(-d) - 1.) ).real();
                // if (split_g) {
                //     dS += (g * neighbourSum[2] * exp(I * (phi(0, x) - phi(1, x))) * (exp(I * d) - 1.)).real();
                // }
                // else
                dS += g * (polar1((3 * phi(0, x) - phi(1, x))) * (polar1(3 * d) - 1.)).real();
                ...
                dS = -2. * kappa[1] * (polar1(-phi(1, x)) * neighbourSum[1] * (polar1(-d) - 1.)).real();
                // if (split_g)
                //     dS += (g * neighbourSum[2] * exp(I * (phi(0, x) - phi(1, x))) * (exp(-I * d) - 1.)).real();
                // else
                dS += g * (polar1((3 * phi(0, x) - phi(1, x))) * (polar1(-d) - 1.)).real();
