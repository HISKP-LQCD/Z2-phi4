...
The plots for each ensemble are
 ...
 - BH four point function $$C_4^{BH}=\frac{\langle \phi_0(\frac{T}{2})\phi_1(t)\phi_1(\frac{T}{8}) \phi_0(0)\rangle}
{\langle \phi_0(\frac{T}{2}) \phi_0(0)\rangle \langle \phi_0(\frac{T}{2})\phi_1(t)\phi_1(\frac{T}{8}) \rangle} -1$$
...
colors <- c("a_0" = "blue", "a_1" = "red", "a_01"="black")

#gg <- ggplot(df, aes(x=msq0, y=a_0,color ="a_0")) + geom_point() 
#gg <- gg +geom_errorbar(aes(ymin=a_0-a_0err, ymax=a_0+a_0err,  color="a_0"), width=0.005)  
#gg <- gg+ geom_point( aes(x=msq0, y=a_1, color="a_1")) 
#gg <- gg +geom_errorbar(aes(ymin=a_1-a_1err, ymax=a_1+a_1err , color="a_1"),  width = 0.005)  

gg <- ggplot(df, aes(x=msq0, y=a_01,color ="a_01"))
gg <- gg+ geom_point( aes(x=msq0, y=a_01, color="a_01")) 
gg <- gg +geom_errorbar(aes(ymin=a_01-a_01err, ymax=a_01+a_01err , color="a_01"),  width = 0.005) 
gg <- gg+labs( color = "Legend") +    scale_color_manual(values = colors)


gg <- gg+ labs(x = TeX('a m_0^2'), y= TeX('$a$') )
